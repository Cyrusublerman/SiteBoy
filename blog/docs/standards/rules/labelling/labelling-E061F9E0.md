<!-- generated: do not edit -->

---
id: labelling-E061F9E0
schema_version: 1.0.0
category: labelling
modality: MUST
statement: Enforce the tag registry for all stored metadata tags.
rationale: Registry enforcement prevents uncontrolled tag sprawl in the archive.
scope: 
  - algorithm
applies_to: []
excludes: []
decidable: judgment
confidence: 0.3
consensus: 1
priority: 550
movements: []
medium: []
conflicts_with: []
supersedes: []
descriptive_origin: false
suppressed_by: null
tags: []
detector:
  kind: none
---

# labelling-E061F9E0

**MUST:** Enforce the tag registry for all stored metadata tags.

## Rationale

Registry enforcement prevents uncontrolled tag sprawl in the archive.

## Sources

- **file:///home/aeinoder/Documents/Cursor/SiteBoy/blog/ideas/tools/external/notebook_decomposition_publishing_system_design_doc.md**
  - weight: 0.6
  - sourced: local
  > file:///home/aeinoder/Documents/Cursor/SiteBoy/blog/ideas/tools/external/notebook_decomposition_publishing_system_design_doc.md
