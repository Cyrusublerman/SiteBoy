...
**[RESOLVED] [BUG] `velocityGrowth` causes simulation divergence**
*Fix: Per-frame speed cap added — outer circles capped at `maxOuterSpeed = size × 0.3`; inner balls capped at `maxSpd = Math.max(parent.r − inner.r, 1)`; both applied before position update each frame.*
...
*Fix: `export: { png: true, gif: false, webm: false }` added; GIF/WebM disabled for non-deterministic infinite animation.*
...
When `fibIndexForCanvas = 15`, the actual canvas size becomes 987×987, but `SCRIPT_CONFIG.canvas.width/height` remain 610. The host may use these values for initial canvas sizing and not resize on param change. Actual resize depends on host's P5 canvas management. Documented as a known limitation.

## NOTE

**[RESOLVED] [DESIGN] `_fibSeq` declared twice**
