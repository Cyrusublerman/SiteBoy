...
Inverts all RGB channels (bitwise complement), producing a photographic negative.
...
| Module type | pixel |
| Source file | `assets/js/tools/processors/distort/nodes/colour/InvertNode.js` |
...
### Process
1. For each pixel: `dstR = 255 − srcR`, `dstG = 255 − srcG`, `dstB = 255 − srcB`.
2. Copy alpha unchanged.

### Output
Colour-inverted RGBA image.

### Preview strategy
No reduction — O(n), trivial.
...
ETCH — inverts the Sobel edge map to produce white lines on dark background.
