...
with $\alpha$ typically set to the golden angle ($\alpha \approx 137.5°$), provides the two-dimensional basis for the dome. To extend this into three dimensions, a vertical mapping is introduced, assigning each point an elevation $z_k$.
...
1. **Phyllotactic radius:** $r_k = C\sqrt{k}$

2. **Divergence angle:** $\theta_k = k \cdot \alpha$, where $\alpha = \frac{360°}{\tau^2}$ and $\tau = \frac{1+\sqrt{5}}{2}$

3. **Boundary condition for $C$:** $C = \frac{R}{\sqrt{N}}$

4. **Vertical mapping (index-based):** $z_k = h_{\max}\left(1 - \frac{k}{N}\right)$

5. **Vertical mapping (radius-based):** $z(r) = h_{\max}\left(1 - \frac{r}{R}\right)$
