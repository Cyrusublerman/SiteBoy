/**
 * DISTORT — Flow Field Node GPU Shaders
 *
 * GPU pattern: gather (noise-based flow advection)
 * Advects pixel sample positions along a noise-defined vector field.
 * Multiple advection steps are approximated in a single pass by accumulating
 * displacement from the noise field evaluated at successive positions.
 *
 * See: nodes/warp/FlowFieldNode.js
 *
 * Binding layout:
 *   @binding(0) Uniforms { uWidth, uHeight, uNoiseScale, uOctaves, uStrength, uCurl,
 *                          uAdvectSteps, uLacunarity, uGain, uFrame, _pad, _pad2 }
...
export const gpuBindings = {
  uniforms: {
    uNoiseScale: 'f32', uOctaves: 'f32', uStrength: 'f32', uCurl: 'f32',
    uAdvectSteps: 'f32', uLacunarity: 'f32', uGain: 'f32', uFrame: 'f32',
  },
  multiPass: false,
  uniformMap: p => ({
    uNoiseScale: p.noiseScale, uOctaves: p.octaves, uStrength: p.strength,
    uCurl: p.curl, uAdvectSteps: p.advectSteps, uLacunarity: p.lacunarity,
    uGain: p.gain, uFrame: p.frame,
  }),
};
