...
/** internalBlend: pattern overlay compositing (not the stack blendMode). */
export const MoireNode = createEffectModule({
  ...
  apply(src, dst, w, h, p) {
    dst.set(moireRGBA(src, w, h, p.wavelength1, p.angle1, p.wavelength2, p.angle2, p.combineMode.toLowerCase(), p.internalBlend.toLowerCase()));
  }
});
