...
export const TruchetNode = createEffectModule({
  ...
  category: 'PATTERN',
  params: {
    tileSize:    { label: 'TILE SIZE',    min: 5, max: 100, step: 1,   value: 20,         tier: 3, previewMax: 40, unit: 'px' },
    strokeWidth: { label: 'STROKE W',     min: 0.5, max: 15, step: 0.5, value: 3,          tier: 3, previewMax: 8,  unit: 'px', driveable: true },
    blendMode:   { label: 'BLEND',        type: 'select', options: ['MULTIPLY', 'SCREEN', 'OVERLAY'], value: 'MULTIPLY', tier: 4 }
  },
  apply(src, dst, w, h, p, ctx) {
    const seed = ctx?.nodeSeed ?? 0;
    dst.set(truchetRGBA(src, w, h, p.tileSize, p.strokeWidth, p.blendMode.toLowerCase(), seed));
  }
});
