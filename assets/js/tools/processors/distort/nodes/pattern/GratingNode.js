...
export const GratingNode = createEffectModule({
  ...
  params: {
    ...
    phase:       { label: 'PHASE',      min: 0, max: 1,   step: 0.01, value: 0,   tier: 3, driveable: true },
    angle:       { label: 'ANGLE',      min: 0, max: 360, step: 1,   value: 0,   tier: 4, unit: 'deg', driveable: true },
    spiralRate:  { label: 'SPIRAL RATE', min: 0.1, max: 10, step: 0.1, value: 1,  tier: 5, driveable: true },
    blendMode:   { label: 'BLEND',      type: 'select', options: ['MULTIPLY', 'SCREEN', 'REPLACE'], value: 'MULTIPLY', tier: 4 }
  },
  apply(src, dst, w, h, p) {
    dst.set(gratingRGBA(src, w, h, p.gratingType.toLowerCase(), p.wavelength, p.phase, p.angle, p.spiralRate, p.blendMode.toLowerCase()));
  }
});
