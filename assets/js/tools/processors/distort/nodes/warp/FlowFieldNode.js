...
export const FlowFieldNode = createEffectModule({
  ...
  params: {
    ...
    octaves:     { value: 3,   min: 1,   max: 8,    step: 1,    label: 'OCTAVES',     tier: 4, driveable: true, unit: 'n' },
  ...
  apply(src, dst, w, h, p, ctx) {
    ...
    dst.set(flowFieldWarp(src, w, h, p.noiseScale, p.octaves, p.lacunarity, p.gain, str, p.curl, adv, noise, interp));
  }
});
