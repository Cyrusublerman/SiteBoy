...
export const AdvectionNode = createEffectModule({
  ...
  params: {
    ...
    steps:        { value: 5,  min: 1,   max: 30, step: 1,   label: 'STEPS',      tier: 3, previewMax: 3, driveable: true, unit: 'n' },
    speed:        { value: 2,  min: 0.1, max: 20, step: 0.1, label: 'SPEED',      tier: 3, driveable: true, unit: 'n' },
    noiseScale:   { value: 3,  min: 0.1, max: 20, step: 0.1, label: 'NOISE SC',   tier: 4, driveable: true, unit: 'n', when: { param: 'velocityType', equals: 'noise' } }
  },
  apply(src, dst, w, h, p, ctx) {
    ...
    dst.set(advectionWarp(src, w, h, p.velocityType, st, p.speed, p.noiseScale, noise, interp));
  }
});
