...
export const SobelNode = createEffectModule({
  ...
  params: {
    ...
    rampSource: { label: 'RAMP SRC', value: 'NORMALISED_MAGNITUDE', type: 'select', options: ['RAW_MAGNITUDE', 'NORMALISED_MAGNITUDE', 'POST_THRESHOLD_VALUE'], tier: 6 },
    rampSpace:  { label: 'RAMP SPACE', value: 'RGB', type: 'select', options: ['RGB', 'HSV'], tier: 6 },
    rampClamp:  { label: 'CLAMP BELOW THRESHOLD', value: 1, tier: 6, type: 'internal' }
  },
  extendedControls: [
    {
  ...
  apply(src, dst, w, h, p, ctx, modulate) {
    ...
    const n = w * h;
    for (let i = 0; i < n; i++) {
      const threshold = modulate ? modulate('threshold', i, ctx) : p.threshold;
      const raw = grey[i * 4];
      const t = (clamp && raw < threshold) ? 0 : raw / 255;
