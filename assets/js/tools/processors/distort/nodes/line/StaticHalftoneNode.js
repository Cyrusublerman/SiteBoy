...
export const StaticHalftoneNode = createEffectModule({
  ...
  params: {
    ...
    frequency:     { label: 'FREQUENCY',  min: 5, max: 300,  step: 1,    value: 60,  tier: 3, driveable: true },
    sampleStep:    { label: 'DENSITY',    min: 0.5, max: 5,  step: 0.25, value: 1,   tier: 4 },
    phaseOffset:   { label: 'PHASE',      min: 0, max: 6.28, step: 0.01, value: 0,   tier: 4, driveable: true },
    phaseInc:      { label: 'PHASE INC',  min: 0, max: 3.14, step: 0.01, value: 0,   tier: 5, driveable: true },
    ampCurve:      { label: 'AMP CURVE',  type: 'select', options: ['LINEAR', 'EXPONENTIAL', 'LOGARITHMIC', 'SIGMOID'], value: 'LINEAR', tier: 4 },
    curveStrength: { label: 'CURVE STR',  min: 0.5, max: 5, step: 0.1,  value: 2,   tier: 5 },
  ...
  applyVector(src, w, h, p) {
    ...
    const set = applyStaticDisplacement({
      ...
      phaseOffset: p.phaseOffset, phaseIncrement: p.phaseInc,
