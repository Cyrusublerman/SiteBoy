...
export const InterferenceNode = createEffectModule({
  ...
  params: {
    ...
    iridescence:   { label: 'IRIDESCENCE', min: 0,   max: 2,   step: 0.05, value: 1,   tier: 3, driveable: true, unit: 'n' },
    blendAmt:      { label: 'BLEND',       min: 0,   max: 1,   step: 0.01, value: 0.5, tier: 3, driveable: true, unit: '0–1' }
  },
  apply(src, dst, w, h, p) {
    const thick = p.filmThickness + p.frame * 2;
    dst.set(thinFilmInterferenceRGBA(src, w, h, thick, p.viewAngle, p.iridescence, p.blendAmt));
  }
});
