...
export const CellularAutomataNode = createEffectModule({
  ...
  params: {
    ...
    steps:     { label: 'STEPS',      min: 1,   max: 500, step: 1,   value: 50,  tier: 3, previewMax: 20 },
    threshold: { label: 'INIT THRESH', min: 0,  max: 255, step: 1,   value: 128, tier: 4, driveable: true },
    blendAmt:  { label: 'BLEND',      min: 0,   max: 1,   step: 0.01, value: 0.5, tier: 3, driveable: true }
  },
  apply(src, dst, w, h, p, ctx) {
    const steps = ctx?.quality === 'preview' ? Math.min(p.steps, 20) : p.steps;
