...
/** internalBlend: noise layer compositing (not the stack blendMode). */
export const PerlinOverlayNode = createEffectModule({
  ...
  params: {
    ...
    internalBlend: { label: 'BLEND',    type: 'select', options: ['ADD', 'MULTIPLY', 'SCREEN', 'OVERLAY'], value: 'ADD', tier: 4 }
  },
  apply(src, dst, w, h, p, ctx) {
    const noise = new PerlinNoise(ctx?.nodeSeed ?? 42);
    dst.set(perlinOverlayRGBA(src, w, h, p.scale, p.octaves, p.strength, p.internalBlend.toLowerCase(), noise));
  }
});
