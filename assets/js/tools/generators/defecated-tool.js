/**
 * Defecated Tool - Morphing text with gooey blur effects
 * 
 * Features:
 * - Cycles through multiple Google Fonts
 * - Gooey morphing effect using blur + threshold shader
 * - Configurable text lines, layout, and timing
 * - Canvas sizing modes (fit/fill/actual)
 * - Full animation export
 * 
 * @version 2.0.0 - Complete ToolBase implementation
 */
...
export const TOOL_CONFIG = {
    ...
    animation: {
        type: 'infinite',
        loopFrames: 0,  // Font cycling makes this infinite
        defaultFps: 60,
        canPrerender: false  // p5.js in iframe, can't pre-render
    },
    
    sidebar: [
        ['TEXT', [
            ['Content', [
                ['text', 'Line 1', 'HAVE YOU', { key: 'line1' }],
                ['text', 'Line 2', 'DEFECATED', { key: 'line2' }],
                ['text', 'Line 3', 'RECENTLY?', { key: 'line3' }]
            ]],
            ['Layout', [
                ['slider', 'Width', 0.5, 0.95, 0.05, { key: 'targetWidth', value: 0.85, withNumber: true }],
                ['slider', 'Height', 0.5, 0.9, 0.05, { key: 'maxHeight', value: 0.75, withNumber: true }],
                ['slider', 'Line Gap', 0, 0.02, 0.001, { key: 'lineGap', value: 0.005, withNumber: true }]
            ]]
        ]],
        ['ANIMATION', [
            ['Timing', [
                ['slider', 'Morph Time (ms)', 800, 3000, 100, { key: 'morphTime', value: 1800, withNumber: true }],
                ['slider', 'Power Curve', 2, 10, 1, { key: 'power', value: 6, withNumber: true }]
            ]],
            ['Effects', [
                ['slider', 'Blur Max', 5, 40, 1, { key: 'blurMax', value: 24, withNumber: true }]
            ]],
            ['Display', [
                ['toggle', 'Options', ['Show Debug'], { key: 'displayOptions', selectedValues: [] }]
            ]]
        ]],
        // EXPORT tab auto-injected by ToolBase when animation config present
    ],
    
    canvas: {
        width: 800,
        height: 600,
        modes: ['fit', 'fill', 'actual']  // Canvas sizing modes
    },
    
    onInit(values, tool) {
        window.debugLog('TOOLS', 'Defecated tool initialized');
        this.iframe = null;
        this.p5Config = this.buildP5Config(values);
        this.startSketch();
    },
    
    onUpdate(key, value, values, tool) {
        // Rebuild config and restart sketch on any parameter change
        if (['line1', 'line2', 'line3', 'targetWidth', 'maxHeight', 'lineGap', 
             'morphTime', 'power', 'blurMax', 'displayOptions'].includes(key)) {
            this.p5Config = this.buildP5Config(values);
            this.restartSketch();
        }
    },
    ...
    onDraw(ctx, canvas, values) {
        // Canvas is replaced by iframe, show placeholder only if iframe not present
        if (!this.iframe || !this.iframe.parentNode) {
            ...
            ctx.fillText('Loading p5.js sketch...', canvas.width / 2, canvas.height / 2);
        }
    },
    
    buildP5Config(values) {
        return {
            lines: [
                values.line1 || 'HAVE YOU',
                values.line2 || 'DEFECATED',
                values.line3 || 'RECENTLY?'
            ],
            targetWidth: values.targetWidth || 0.85,
            maxTotalHeight: values.maxHeight || 0.75,
            lineGap: values.lineGap || 0.005,
            morphTime: values.morphTime || 1800,
            power: values.power || 6,
            blurMax: values.blurMax || 24,
            showDebug: (values.displayOptions || []).includes('Show Debug'),
            canvasWidth: values.canvasWidth || 800,
            canvasHeight: values.canvasHeight || 600
        };
    },
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #debug {
      ...
      z-index: 1000;
      white-space: pre;
      display: ${config.showDebug ? 'block' : 'none'};
    }
  </style>
</head>
<body>
...
<\/script>
    ...
    startSketch() {
        ...
        if (!canvasArea) {
            window.debugLog('TOOLS', 'No canvas area found');
            return;
        }
        ...
        doc.open();
        doc.write(this.generateHTML(this.p5Config));
        doc.close();
        
        window.debugLog('TOOLS', 'Defecated sketch started');
    },
...
export class DefecatedTool {
    ...
    render() {
        try {
            ...
            window.debugLog('TOOLS', '✅ DefecatedTool rendered');
        } catch (error) {
            console.error('❌ DefecatedTool error:', error);
        }
    }
...
export default DefecatedTool;

window.debugLog('TOOLS', '✅ DefecatedTool module loaded');
