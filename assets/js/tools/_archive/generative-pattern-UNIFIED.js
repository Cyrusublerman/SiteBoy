...
(function() {
    ...
    var TOOL_CONFIG = {
        title: 'GENERATIVE PATTERN (UNIFIED)',
        ...
        onInit: function(v) {
            ...
            wire(this, 'playPause', function() { togglePlay(self); });
            wire(this, 'reset', function() { rebuild(v, self); self.draw(); });
        ...
        onRenderFrame: function(i) { state.time = i / 60; this.draw(); }
    };
    ...
    function buildVelocityField(v, tool) {
        ...
        state.velocityField = {
            w: Math.floor(tool.canvas.width / 4),
            h: Math.floor(tool.canvas.height / 4)
        };
    ...
    function GenerativePatternTool(container, deps) {
    ...
    GenerativePatternTool.prototype.destroy = function() {
        if (state.animator) state.animator.destroy();
